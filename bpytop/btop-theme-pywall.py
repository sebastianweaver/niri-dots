...
# original: # original: https://github.com/LierB/dotfiles/blob/d8c87da7bb43da2c1517d5c4be03a1b4bf1d9424/.config/wal/templates/btop
...
def get_args():
    parser = argparse.ArgumentParser(description="create btop theme file with pywal")
    parser.add_argument(
        "-n",
        "--name",
        required=True,
...
def transform_color(colors):
    ...
    new_colors = dict(
        sum(
            [
                [(f"color{i + 1}", color), (f"color{i + 9}", color)]
...
if __name__ == "__main__":
    ...
    wallpaper = os.path.expanduser(
        os.path.join(args.wp_dir, args.name + "." + args.file_type)
    )
